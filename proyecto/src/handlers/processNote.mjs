...
export const handler = async (event) => {
  ...
  // var noteData = JSON.parse(event.pathParameters); // Convertimos de JSON a objeto javascript
  ...
  // var noteId = noteData ? noteData.noteId : null;
  ...
  try {
    ...
    //  Obtengo el item cuyo texto de quiere reproducir
    var item = await libreria.getNote(userId, noteId); 
    console.log(item);
    // Obtengo el texto
    const Texto = item[0].text;
    console.log(Texto)
    //Obtengo el buffer de datos del mp3
    var buffer = await libreria.textToSpeech(Texto);
    console.log(buffer)
    // Obtengo la URL prefirmada
    var URL = await libreria.uploadToS3(buffer, noteId)
    console.log(URL);
    // Traduzco y actualizo dynamoDB
    var translatedText = await libreria.translateAndUpdateNote(userId, noteId, Texto, "en", URL);
